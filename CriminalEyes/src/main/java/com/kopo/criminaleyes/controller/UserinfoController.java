...
import com.kopo.criminaleyes.service.IUserinfoService;
import com.kopo.criminaleyes.service.impl.UserinfoService;
import com.kopo.criminaleyes.util.CmmUtil;
import com.kopo.criminaleyes.util.EncryptUtil;
...
public class UserinfoController {
    ...
    public String userRegForm() {
        log.info("{}.use" +
                "userRegForm", this.getClass().getName());
    ...
    public MsgDTO loginProc(HttpServletRequest request, HttpSession session) {
        ...
        try {
            ...
            // 로그인를 위해 아이디와 비밀번호가 일치하는지 확인하기 위한 userinfoService 호출하기
            UserinfoDTO rDTO = userinfoService.getLogin(pDTO);

            // 로그인을 성공했다면, 회원아이디 정보를 session에 저장함
            /*
             * 세션은 톰캣(was)의 메모리에 존재하며, 웹사이트 접속 시 사용자(연결된 각각)마다 메모리에 값을 올린다.
             *
             * 예) 톰캣에 100명의 사용자가 로그인했다면, 사용자 각각 회원아이디를 메모리에 저장해야 한다.
             *    => 메모리에 저장되는 회원정보는 최대 100개이다.
             *    => 메모리에 저장된다는 특징 때문에 서버를 재시작하면 서버가 다운되는 현상이 있을 수 있기 때문에,
             *       최소한의 회원정보만을 저장하는 것을 권장한다.
             *
             * 스프링에서 세션을 사용하기 위해서는 함수의 파라미터에 HttpSession session 변수를 선언해야 한다.
             * 세션에 저장할 수 있는 값은 문자열이나 url만 되는 것이 아니라, 모든 종류의 Java 객체를 저장할 수 있다.
             * 저장된 객체는 서버 메모리에 존재하기 때문에 jsp, controller에서 쉽게 불러서 쓸 수 있다.
             */
    ...
    @PostMapping(value = "searchUserIdProc") // Hyeogeon Lee
    ...
    #
    #   무조건 임의로 받은 정보는 DTO에 저장하기 위해 임시로 String 변수에 저장함
    #
    #####################################################################
    */
        ...
        UserinfoDTO rDTO = Optional.ofNullable(
                userinfoService.searchUserIdOrPasswordProc(pDTO) // 아이디 찾기
    ...
    public String searchPassword(HttpSession session) {
        ...
        // 비밀번호 재설정하는 화면은 보안을 위해 생성한 NEW PASSWORD 세션 삭제
    ...
    public String searchPasswordProc(HttpServletRequest request, ModelMap model, HttpSession session) throws Exception {
        ...
        /*
         * ##########################################################
         * 웹(회원정보 입력단말)에서 받은 정보를 DTO에 저장하기!!
         *
         * 무조건 웹으로 받은 정보는 DTO에 저장해야 한다고 이해하면 됨
         * ##########################################################
         */
        UserinfoDTO pDTO = new UserinfoDTO();
        pDTO.setUserId(userId);
        ...
        // 비밀번호 재설정하는 화면은 보안을 위해 반드시 NEW_PASSWORD 세션이 존재해야 접속 가능하도록 구현
     ...
     * 비밀번호 찾기 로직 수행
     * <p>
     * 아이디, 이름, 이메일 일치하면, 비밀번호 재발급 화면 이용
     */
    ...
    public String newPasswordProc(HttpServletRequest request, ModelMap model, HttpSession session) throws Exception {
        ...
        if (!newPassword.isEmpty()) { //정상 접근
            ...
            /*
             * ##########################################################################################
             *   웹(회원정보 입력화면)에서 받는 정보를 String에 저장!!
             *
             *   무조건 웹으로 받은 정보는 DTO에 저장하기 위해 임시로 String 변수에 저장함
             * ##########################################################################################
             */
            ...
            // 비밀번호 재설정하는 화면은 보안을 위해 생선한 NEW_PASSWORD 세션 삭제
            session.setAttribute("NEW_PASSWORD", "");
            session.removeAttribute("NEW_PASSWORD");
