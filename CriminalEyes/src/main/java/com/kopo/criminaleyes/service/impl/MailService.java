...
public class MailService implements IMailService {
    ...
    @Value("${spring.mail.username}") // 보낼 이메일 비번 정하면 자동으로 될거임
    private String fromMail;

    @Override
    public int doSendMail(MailDTO pDTO) {

        //로그 찍기(주후 찍은 로그를 통해 이 함수에 접근했는지 파악하기 용이하다.)
        log.info("{}.doSendMail start!", this.getClass().getName());

        // 메일 발송 성공여부(발송성공 : 1 / 발송실패 : 0)
        int res = 1;

        //전달 받은 DTO로부터 데이터 가져오기(DTO객체가 메모리에 올라가지 않아 Null이 발송할 수 있기 때문에 에러방지차원으로 if문 사용함
        if (pDTO == null) {
            pDTO = new MailDTO();
        }
