...
import com.kopo.criminaleyes.service.INoticeService;
import com.kopo.criminaleyes.service.impl.NoticeService;
...
public class NoticeController {
     ...
     * 게시판 리스트 보여주기
     * <p>
     * GetMapping(value = "notice/noticeList ") => GET방식을 통해 접속되는 "URL이 notice/noticeList 경우 아래 함수를 실햄함
     */
     ...
     * JSON 구조로 결과 메시지를 전송하기 위해 @ResponseBody 어노테이션을 추가함
     */
    ...
    public MsgDTO noticeInsert(HttpServletRequest request, HttpSession session) throws Exception{
        ...
        try {
            // 로그인된 사용자 아이디를 가져오기
            // 로그인을 아직 구현하지 않기 때문에 공지사항 리스트에서 로그인된 사용자의 Session 값을 저장함
            ...
            // 반드시, 값이 제대로 들어오는지 로그를 찍어야 문제가 없는지 파악이 가능함
            // 무조건 찍어야 하는 로그 -> 반드시 찍자!!!
    ...
    public MsgDTO noticeUpdate(HttpSession session, HttpServletRequest request) {
        ...
        try {
            ...
            String nSeq = CmmUtil.nvl(request.getParameter("nSeq")); // 기본키(PK)
            String title = CmmUtil.nvl(request.getParameter("title")); // 제목
            String noticeYn = CmmUtil.nvl(request.getParameter("noticeYn")); // 공지글 여부
            String contents = CmmUtil.nvl(request.getParameter("contents")); // 내용
            ...
            /*
             * 값 전달은 DTO 객체를 생성하여 저장하기 위해 DTO 객체에 값을 넣는다.
             */
            NoticeDTO pDTO = new NoticeDTO();
            pDTO.setUserId(userId);
            pDTO.setNoticeSeq(nSeq);
